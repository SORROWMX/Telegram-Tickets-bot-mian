...
class User:
    ...
    def _get_by_uid(self) -> UserData:
        ...
        cursor.execute("SELECT * FROM Users_bot WHERE id = ?", (self.uid, ))
    ...
    def _get_by_ticket(self) -> UserData:
        cursor = self.connection.cursor()
        cursor.execute("SELECT * FROM Users_bot WHERE ticketID = ?", (int(self.ticket_id), ))
        result = cursor.fetchone()
        cursor.close()
        if result:
            self.userdata = UserData(result[0], result[1], result[2], result[3])
    ...
    def create(self) -> UserData:
        ...
        try:
            ...
            cursor = self.connection.cursor()
            cursor.execute("INSERT INTO Users_bot (ID, username, ticketID, language) VALUES (?,?,?,?)", (self.uid, self.username, -99999, "en"))
    ...
    def set_language(self, lang:str) -> UserData:
        try:
            cursor = self.connection.cursor()
            cursor.execute("UPDATE Users_bot SET language = ? WHERE ID = ?", (lang, self.uid))
            self.connection.commit()
            cursor.close()
    ...
    def set_ticket(self, ticketID:int):
        cursor = self.connection.cursor()
        cursor.execute("UPDATE Users_bot SET ticketID = ? WHERE ID = ?", (ticketID, self.uid))
        self.connection.commit()
        cursor.close()
